...
namespace crossblog.Controllers
{
    ...
    public class ArticlesController : Controller
    {
        ...
        {
            var titleLikeClause = "%"+ title + "%";
            var articles = await _articleRepository.Query().Where(a => EF.Functions.Like(a.Title, titleLikeClause) || EF.Functions.Like(a.Content, titleLikeClause)).Take(20).ToListAsync();
        ...
        public async Task<IActionResult> Post([FromBody]ArticleModel model)
        {
            ...
            return Created($"articles/{article.Id}", result);
        }
